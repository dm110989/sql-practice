...
.value_counts(normalize = True) параметр normalize = 
Сначала считает количество каждого уникального значения (обычный value_counts)

Потом делит каждое количество на общее число строк

Возвращает доли (проценты) вместо абсолютных чисел

✔️ Итоговая таблица параметров
Параметр	Что делает	Когда использовать

normalize	превращает количество → долю	анализ долей, процентов
sort	    сортирует результат	почти всегда оставляем True
ascending	направление сортировки	если нужен рост, а не убывание
bins	    группирует числа по интервалам	анализ распределений
dropna	    учитывать NaN или нет	если пропуски важны
